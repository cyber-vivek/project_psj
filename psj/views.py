...
def index(request):
    return render(request,"psj/index.html")


def result(request):
    query = request.GET.get('query','hello')
    ...
    for tup in result_set:
        if(tup[1]==0):
            continue
        href = "psj\data1\\"+str(tup[0])
        results.append({"doc_name":str(tup[0].capitalize()),"weight":str(tup[1]),"href":href})
    results.reverse()
    paramt = {"q":query,"results":(results)}
    return render(request,"psj/respage.html",paramt)



# Precomutation for files present in local storage
...
address="D:\explo proj\project-psj\project_psj\psj\static\psj\data1"
